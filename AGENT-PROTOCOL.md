...
3. 每条必须有来源 URL
4. 修改源配置要记录理由到 D1 `ops_state` 或本地 `data/ops-state.json` 工作副本
...
| "日报质量下降了" | 记录到 `ops_state` → 在下次 GENERATE 阶段调整策略 |
...
```bash
npx tsx scripts/ops-runner.ts publish       # D1-only 后不再提交运行数据
curl -fsS https://reado-station-web.cing-self.workers.dev/api/health
npx tsx scripts/ops-runner.ts publish-lark  # 飞书群推送（D1 健康确认后才发）
```

**顺序很重要**：采集/摘要必须先通过 D1 API 写入成功，再确认 `/api/health`，最后发飞书消息。不要把 `data/` 或 `site/src/data/*.json` 提交到 Git。
...
| BUILD | 脚本 | 调构建工具，纯机械 |
| PERSIST | 脚本 | 写 ops-state 工作副本 / 同步 D1，纯机械 |
| PUBLISH | 脚本 | D1 健康确认 + 飞书发送，纯机械 |
