...
- 群里有人问"今天的情报/日报有吗" → 读 `data/latest-digest.md`，把完整内容发到群里，不要自己概括
- 群里有人问"为什么没有 XXX 的消息" → 先在已采集数据中搜索 → 没有则联网搜索确认该事件是否真实存在 → 如果真实存在，找到原始来源 → 评估能否接入 reado 信息源 → 回复用户完整结果
- 群里有人反馈内容质量问题 → 记录到 ops-state.json → 在下次运营循环中调整
- 不要回复"我不确定"后就结束 — 不确定就去查
