...
describe('profile enrichment jobs', () => {
  ...
  it('uses BIGMODEL_OKIT_KEY for profile search and model selection when explicit providers are not configured', async () => {
    ...
    const modelInputLog = writes.find((statement) => statement.sql.includes('INSERT INTO execution_logs') && statement.bindings.includes('profile_asset_model_input'))
    const modelOutputLog = writes.find((statement) => statement.sql.includes('INSERT INTO execution_logs') && statement.bindings.includes('profile_asset_model_output'))
    expect(modelInputLog).toBeTruthy()
    expect(modelOutputLog).toBeTruthy()
    expect(String(modelInputLog?.bindings[8])).toContain('Use web search evidence')
    expect(String(modelInputLog?.bindings[8])).toContain('https://www.anthropic.com')
    expect(String(modelOutputLog?.bindings[8])).toContain('Official website selected by the model.')
    const profileWrite = writes.find((statement) => statement.sql.includes('INSERT INTO channel_profiles'))
